#####################################################################################
# Mask-based_CNN_3CL:
# Training the Mask-based CNN model using 3CL for speech enhancement,
...
model = Model(inputs=[input_noisy, input_noise_component, input_speech_component], outputs=[n_tilde, n_tilde_residual_power, s_tilde])
model.summary()
...
model.compile(optimizer=adam_wn, loss=['mean_squared_error', my_loss, 'mean_squared_error'], loss_weights=[0.1, 0.8, 0.1], metrics=['accuracy'])
...
best_weights = './training_results/mask_CNN_3CL' + '.h5'
...
hist = model.fit([train_input_noisy, train_pure_noise, train_clean_speech], [train_residual_noise_power_target, train_pure_noise, train_clean_speech], epochs=nb_epochs, verbose=2, batch_size=batch_size, shuffle=True, initial_epoch=0,
                      callbacks=[reduce_LR, stop_str, model_save],
                      validation_data=[[vali_input_noisy,vali_pure_noise,vali_clean_speech], [vali_residual_noise_power_target, vali_pure_noise, vali_clean_speech]]
                      )
